...
CREATE TABLE IF NOT EXISTS `eseitfgmanager`.`Alumno` (
  ...
  `contrasenhaAl` VARCHAR(30) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `eseitfgmanager`.`Profesor` (
  ...
  `email` VARCHAR(45) NOT NULL,
  `contrasenhaPr` VARCHAR(30) NOT NULL,
  `nombre` VARCHAR(100) NOT NULL,
...
-- -----------------------------------------------------
-- Table `eseitfgmanager`.`Profesor_tutorizadoPor_TFG`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `eseitfgmanager`.`Profesor_tutorizadoPor_TFG` ;

CREATE TABLE IF NOT EXISTS `eseitfgmanager`.`Profesor_tutorizadoPor_TFG` (
  `Profesor_dniProfesor` VARCHAR(9) NOT NULL,
  `TFG_idTFG` VARCHAR(9) NOT NULL,
  `Profesor_dniProfesorCotutor` VARCHAR(9) NULL DEFAULT NULL,
  PRIMARY KEY (`Profesor_dniProfesor`, `TFG_idTFG`, `Profesor_dniProfesorCotutor`),
  INDEX `fk_Profesor_has_TFG_TFG1_idx` (`TFG_idTFG` ASC),
  INDEX `fk_Profesor_has_TFG_Profesor1_idx` (`Profesor_dniProfesor` ASC),
  ...
  CONSTRAINT `fk_Profesor_has_TFG_TFG1`
    FOREIGN KEY (`TFG_idTFG`)
    REFERENCES `eseitfgmanager`.`TFG` (`idTFG`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `eseitfgmanager`.`Coordinador` (
  ...
  `email` VARCHAR(45) NOT NULL,
  `contrasenhaC` VARCHAR(30) NOT NULL,
